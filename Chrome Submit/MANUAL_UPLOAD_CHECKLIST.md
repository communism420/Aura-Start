...
Before upload, verify that the ZIP was built with the real Chrome Extension `AURA_GOOGLE_OAUTH_CLIENT_ID` for the published extension ID and the real `AURA_GOOGLE_WEB_OAUTH_CLIENT_ID` for Brave/Chromium fallback support. The Web OAuth client must authorize the final JavaScript origin `https://<extension-id>.chromiumapp.org` and the exact final redirect URI `https://<extension-id>.chromiumapp.org/oauth2`. Confirm the final ZIP requests only `https://www.googleapis.com/auth/drive.appdata`, not full Drive or `drive.file`, and does not contain any unapproved bundled OAuth client ID.
