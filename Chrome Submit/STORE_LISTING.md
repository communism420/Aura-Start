# Aura Start Chrome Web Store Listing

## Extension Name

Aura Start

## Short Description

A local-first, private, exportable start page for your Chromium new tab.

## Detailed Description

Aura Start replaces the Chromium new tab page with clean groups of links, local-first storage, and export-first data ownership.

Create groups, save links, search by title, URL, description, tags, and supported modifiers, and move faster with Command Palette and keyboard shortcuts. Aura Start includes Import from A Fine Start export codes, Full Backup JSON, Browser Bookmarks HTML, Markdown, CSV, and A Fine Start-compatible export code. Restore points help protect destructive actions, and Duplicate Finder scans local links read-only before any user-confirmed deletion.

Aura Start has no required account, no analytics, no tracking, no ads, and no backend. Optional Google Drive backup/sync is off by default and uses only the hidden Google Drive appDataFolder scope for Aura Start's own `aura-start-sync.json` file.

Aura Start is open-source under the MIT License. It is an independent project and is not affiliated with A Fine Start.

## Feature Bullets

- Organize links into clean groups
- Search by title, URL, description, tags, and supported query modifiers
- Import from A Fine Start export codes
- Export Full Backup JSON, Browser Bookmarks HTML, Markdown, CSV, and A Fine Start-compatible code
- Restore points before destructive actions
- Command Palette and keyboard shortcuts
- Duplicate Finder with read-only scan and user-confirmed deletion
- Optional Google Drive appDataFolder backup
- No account, no analytics, no tracking
- Open-source under the MIT License

## Recommended Store Fields

- Category: Productivity
- Language: English
- Website URL: `https://communism420.github.io/Aura-Start/` after GitHub Pages is published, or `https://github.com/communism420/Aura-Start`
- Support URL: `https://github.com/communism420/Aura-Start/issues` if Issues are enabled, otherwise the repository URL
- Source code URL: `https://github.com/communism420/Aura-Start`
- Privacy policy URL: `https://communism420.github.io/Aura-Start/privacy-policy.html` after GitHub Pages is published

## Screenshot Notes

Upload the PNG files from `Photo/` in filename order. They were captured from the real Aura Start 1.2.0 UI with non-personal demo data.
