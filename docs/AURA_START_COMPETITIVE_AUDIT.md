...
| Google Drive OAuth complexity | Requires real OAuth client IDs and possibly Web OAuth fallback for some Chromium browsers. | Store review and user support can fail on OAuth setup more than on UI. | Keep reviewer notes tight, test final extension ID, and consider a local-only first submission if Drive creates review delays. | P1 |
...
2. Verify final OAuth client configuration.
   ...
   - If Web OAuth fallback is used, that must also be verified with the correct redirect URI.
