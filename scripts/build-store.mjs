...
const deviceOAuthClientSecret = process.env.AURA_GOOGLE_STORE_DEVICE_OAUTH_CLIENT_SECRET?.trim();

if (!deviceOAuthClientSecret) {
  console.error("AURA_GOOGLE_STORE_DEVICE_OAUTH_CLIENT_SECRET is required for Chrome Web Store builds so Brave/Helium can use Google Device OAuth without redirect_uri. Do not use the local unpacked-extension device OAuth secret for store builds.");
  process.exit(1);
}
...
for (const [command, args] of steps) {
  const result = spawnSync(command, args, {
    ...
    env: {
      ...
      AURA_GOOGLE_DEVICE_OAUTH_CLIENT_ID: CHROME_WEB_STORE_DEVICE_OAUTH_CLIENT_ID,
