...
const en = {
  ...
  googleDriveDeviceAuthDescription: "Aura Start will open Google's device sign-in page. If the code is not filled automatically, copy it and paste it on the Google page.",
  googleDriveDeviceAuthExpires: "Expires at {time}",
  googleDriveDeviceAuthInstructions: "Keep Aura Start open while Google confirms the sign-in. Aura Start only requests Drive app data access for its hidden sync file.",
  googleDriveDeviceAuthOpenGoogle: "Open Google sign-in",
  googleDriveDeviceAuthTitle: "Connect Google Drive",
  googleDriveGoogleWindow: "Connect opens Google's authorization directly. In some Chromium browsers Aura Start may show a short Google sign-in code instead; no manual client ID setup is needed in Aura Start.",
...
const translations: Record<AuraLanguage, Record<I18nKey, string>> = {
  ...
  ru: {
    ...
    googleDriveDeviceAuthDescription: "Aura Start откроет страницу входа Google для устройств. Если код не подставится автоматически, скопируйте его и вставьте на странице Google.",
    googleDriveDeviceAuthExpires: "Истекает в {time}",
    googleDriveDeviceAuthInstructions: "Не закрывайте Aura Start, пока Google подтверждает вход. Aura Start запрашивает только доступ Drive app data для скрытого sync-файла.",
    googleDriveDeviceAuthOpenGoogle: "Открыть вход Google",
    googleDriveDeviceAuthTitle: "Подключить Google Drive",
    googleDriveGoogleWindow: "Подключение открывает авторизацию Google напрямую. В некоторых Chromium-браузерах Aura Start может показать короткий код входа Google; вручную настраивать client ID в Aura Start не нужно.",
