...
const en = {
  ...
  googleDriveGoogleWindow: "Connect opens Google's authorization directly. In Chromium browsers that do not support Chrome's built-in sign-in, Aura Start uses its configured Web OAuth fallback; no manual client ID setup is needed in Aura Start.",
...
const translations: Record<AuraLanguage, Record<I18nKey, string>> = {
  ...
  ru: {
    ...
    googleDriveGoogleWindow: "Подключение открывает авторизацию Google напрямую. В Chromium-браузерах без встроенного входа Chrome Aura Start использует настроенный Web OAuth fallback; вручную настраивать client ID в Aura Start не нужно.",
