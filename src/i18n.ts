...
const en = {
  ...
  googleDriveOAuthClientId: "Self-built OAuth Client ID",
  googleDriveOAuthClientIdDescription: "Advanced self-built setup only. The OAuth Client ID identifies the Aura Start app, not your Google account, so Google sign-in cannot supply it automatically. Official builds use the app client configured in the manifest.",
  googleDriveOAuthClientIdPlaceholder: "1234567890-abc.apps.googleusercontent.com",
  googleDriveOAuthRedirectUrl: "OAuth redirect URL",
  ...
  googleDriveSaveOAuthClientId: "Save client ID",
...
const translations: Record<AuraLanguage, Record<I18nKey, string>> = {
  ...
  ru: {
    ...
    googleDriveOAuthClientId: "OAuth Client ID для self-built сборки",
    googleDriveOAuthClientIdDescription: "Только для advanced self-built настройки. OAuth Client ID идентифицирует приложение Aura Start, а не ваш Google-аккаунт, поэтому вход через Google не может выдать его автоматически. Официальные сборки используют client ID приложения из manifest.",
    googleDriveOAuthClientIdPlaceholder: "1234567890-abc.apps.googleusercontent.com",
    googleDriveOAuthRedirectUrl: "OAuth redirect URL",
    ...
    googleDriveSaveOAuthClientId: "Сохранить client ID",
