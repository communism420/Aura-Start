...
describe("Google Drive OAuth flow selection", () => {
  ...
  it("uses Device OAuth in ambiguous Chrome-like browsers when Chrome identity is unsupported", () => {
    expect(
      selectGoogleDriveAuthFlow({
        hasIdentityApi: true,
        hasGetAuthToken: true,
  ...
  it("falls back to Web OAuth when Device OAuth is not configured", () => {
    expect(
      selectGoogleDriveAuthFlow({
        hasIdentityApi: true,
        hasGetAuthToken: true,
  ...
  it("returns unavailable when Chrome identity is unavailable and no Web OAuth fallback is configured", () => {
    expect(
      selectGoogleDriveAuthFlow({
        hasIdentityApi: true,
        hasGetAuthToken: false,
...
describe("Google Drive browser OAuth capability detection", () => {
  ...
  it("identifies plain Chromium and uses Web OAuth fallback", async () => {
    const navigatorValue = {
  ...
  it("identifies ungoogled Chromium variants such as Helium and uses Web OAuth fallback", async () => {
    const navigatorValue = {
...
describe("Google Drive Web OAuth redirect URI", () => {
  ...
  it("builds Chrome's canonical root chromiumapp.org fallback URL when no custom path is used", () => {
    const originalChrome = globalThis.chrome;
    Object.defineProperty(globalThis, "chrome", {
      configurable: true,
      value: {
